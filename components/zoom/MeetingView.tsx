...
export function MeetingView({ players, currentPlayerId }: MeetingViewProps) {
  ...
  return (
    <div className="flex-1 flex items-center justify-center bg-[#0D0D0D] p-4 overflow-hidden">
      <div 
        className="w-full h-full grid gap-2"
        style={{
          gridTemplateColumns: `repeat(${cols}, 1fr)`,
          gridTemplateRows: `repeat(${rows}, 1fr)`,
          maxWidth: displayPlayers.length === 1 ? '800px' : '100%',
          maxHeight: displayPlayers.length === 1 ? '600px' : '100%',
        }}
      >
          ...
          >
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-[#2D2D2D] to-[#1A1A1A]">
              <div 
                ...
                style={{ fontSize: displayPlayers.length <= 4 ? '8rem' : displayPlayers.length <= 9 ? '5rem' : '3rem' }}
              >
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent py-2 px-3">
              <div className="flex items-center justify-center gap-2">
                ...
                <span className={`text-white font-medium truncate ${displayPlayers.length <= 4 ? 'text-base' : 'text-sm'}`}>
                  {player.name}
                </span>
                {player.isHost && (
                  <span className="bg-[#0E72ED] text-white text-xs px-1.5 py-0.5 rounded-full flex-shrink-0">
                    Host
                  </span>
                )}
              </div>
